...
func TestImagePullBackOffRunningSurfacesNotification(t *testing.T) {
	...
	if got, want := messages, regexp.MustCompile("ErrImagePull|ImagePullBackOff"); !want.MatchString(got) {
		t.Errorf("expected the image pull failure reason to be surfaced as a stack notification, got: %q", messages)
	}
	if got, want := messages, "container-0"; !strings.Contains(got, want) {
		t.Errorf("strings.Join(tc.NotificationMessages(jobID), %q) = %q, want containing %q", "\n", got, want)
	}
}
