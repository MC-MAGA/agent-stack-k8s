...
func (t testcase) createClusterQueueWithCleanup() *buildkite.ClusterQueue {
	...
	if err != nil {
		...
		t.Fatalf("t.Buildkite.ClusterQueues.Create(%q, %q, &buildkite.ClusterQueueCreate{\n\tKey: &queueName,\n}) error = %v, want nil", t.Org, t.ClusterUUID, err)
	}
...
func (t testcase) FetchLogs(build api.Build) string {
	...
	for _, edge := range build.Jobs.Edges {
		...
		if !wasJob {
			t.Errorf("edge.Node.(*api.JobJobTypeCommand) = (_, %t), want true", wasJob)
			continue
		}
